...
var myGamePiece = {
    ...
    crashWith: function(otherobj) {
        ...
        // Verifica collisione con l'altro oggetto
        if ((mybottom >= othertop) && (mytop <= otherbottom) && (myright >= otherleft) && (myleft <= otherright)) {
            return; // Collisione rilevata, non aggiornare la posizione
        }
    
        // Verifica che non esca dal canvas
        if (this.tryX < 0) this.tryX = 0; // Bordo sinistro
        if (this.tryX + this.width > myGameArea.canvas.width) this.tryX = myGameArea.canvas.width - this.width; // Bordo destro
        if (this.tryY < 0) this.tryY = 0; // Bordo superiore
        if (this.tryY + this.height > myGameArea.canvas.height) this.tryY = myGameArea.canvas.height - this.height; // Bordo inferiore
    
        // Se non c'è collisione e non esce dai bordi, aggiorna la posizione
        this.x = this.tryX;
        this.y = this.tryY;
    }
};
